...
class ProfileManager:
    def __init__(self, max_profiles=5):
        ...
        self.config_file = 'antarctic_profiles.json'
        self.load_profiles()
...
class AutoClicker:
    ...
    def get_timing_delay(self):
        # Advanced timing system takes precedence
        if self.config.advanced_timing_enabled and self.config.advanced_profile:
            ...
            # Apply micro-jitter for race conditions
            if profile.jitter_range:
                jitter = random.uniform(profile.jitter_range[0], profile.jitter_range[1])
                base_delay += jitter

            # Apply interval configuration to advanced timing - use interval as minimum delay
            interval_delay = self.config.interval / 1000.0  # Convert ms to seconds
            base_delay = max(base_delay, interval_delay)  # Use the larger delay

            return max(0.00001, base_delay)  # Minimum 10 microseconds
        ...
        if self.config.timing_profile == 'human_slow':
            ...
            base_delay = random.uniform(0.001, 0.050)
        elif self.config.timing_profile == 'precise':
            base_delay = 0.001
    ...
    def execute_burst(self, delay=None):
        ...
        try:
            ...
            if self.config.advanced_timing_enabled and self.config.advanced_profile:
                self._execute_advanced_burst(end_time, stats_update_interval)
            else:
                # Legacy burst execution
                config_clicks = self.config.clicks
                config_interval = self.config.interval
                while time.time() < end_time and self.running:
                    clicks_to_send = random.randint(5, 15) if humanize else config_clicks
                    for _ in range(clicks_to_send):
                        if not self.running:
                            break
                        user32.SendMessageW(hwnd, self._msg_down, self._wparam, self._lparam)
                        if not ultra_mode:
                            if humanize:
                                ...
                                time.sleep(config_interval / 1000.0)
                        user32.SendMessageW(hwnd, self._msg_up, 0, self._lparam_up)
                        self.total_clicks_sent += 1
                        self.current_burst_clicks += 1
                        click_count += 1
    ...
    def _execute_advanced_burst(self, end_time, stats_update_interval):
        """Execute burst using advanced timing profiles"""
        profile = self.config.advanced_profile
        hwnd = self.target_hwnd
        click_count = 0
        ...
        while time.time() < end_time and self.running:
            ...
            for i, clicks_in_burst in enumerate(profile.burst_pattern):
                if time.time() >= end_time or not self.running:
                    break

                # Send the specified number of clicks
                for _ in range(clicks_in_burst):
                    if not self.check_window_valid(hwnd) or not self.running:
                        return
                ...
                # Apply delay pattern with interval consideration
                if i < len(profile.delay_pattern) - 1 and self.running:
                    delay = profile.delay_pattern[i]
                    # Apply interval configuration to delay pattern
                    interval_delay = self.config.interval / 1000.0  # Convert ms to seconds
                    delay = max(delay, interval_delay)  # Use the larger delay
                    if delay > 0:
                        time.sleep(delay)

            # Reset pattern if needed
            if time.time() < end_time and self.running:
                # Small pause between pattern cycles
                time.sleep(0.001)
...
class AntarcticGUI(ctk.CTk):
    ...
    def create_status_bar(self):
        status_frame = ctk.CTkFrame(self, fg_color="#2a2a2a", corner_radius=6, height=40)
        status_frame.pack(fill="x", padx=15, pady=10)
        status_frame.pack_propagate(False)

        self.conn_indicator = ctk.CTkLabel(status_frame, text="● OFFLINE", font=("Consolas", 11, "bold"), text_color="#CC0000")
        self.conn_indicator.pack(side="left", padx=15)

        self.burst_indicator = ctk.CTkLabel(status_frame, text="▶ STANDBY", font=("Consolas", 11), text_color="#888888")
        self.burst_indicator.pack(side="left", padx=15)

        self.coords_label = ctk.CTkLabel(status_frame, text="TARGET: [ --- , --- ]", font=("Consolas", 11), text_color="#888888")
        self.coords_label.pack(side="left", padx=15, expand=True)

        self.stats_label = ctk.CTkLabel(status_frame, text="CLICKS: 0 / 0", font=("Consolas", 11), text_color="#888888")
    ...
    def create_sliders(self):
        ...
        self.clicks_slider, self.clicks_label = self.create_clean_slider(sliders_frame, "Clicks", 1, 100, 24, lambda v: setattr(self.clicker.config, 'clicks', int(v)))
        self.interval_slider, self.interval_label = self.create_clean_slider(sliders_frame, "Interval", 1, 200, 10, lambda v: setattr(self.clicker.config, 'interval', int(v)))
        self.duration_slider, self.duration_label = self.create_clean_slider(sliders_frame, "Duration", 0.01, 2.0, 0.30, lambda v: setattr(self.clicker.config, 'duration', float(v)))
        self.delay_slider, self.delay_label = self.create_clean_slider(sliders_frame, "Delay", 0.0, 1.0, 0.0, lambda v: setattr(self.clicker.config, 'auto_burst_delay', float(v)))
    ...
    def create_actions(self):
        ...
        self.humanize_checkbox = ctk.CTkCheckBox(humanize_frame, text="Humanization", font=("Consolas", 10), command=self.toggle_humanization, checkbox_width=18, checkbox_height=18, corner_radius=4, border_width=1)
        self.humanize_checkbox.pack(side="left")

        # Advanced timing toggle (for burst variations) - simplified
        self.advanced_checkbox = ctk.CTkCheckBox(humanize_frame, text="Burst Variations", font=("Consolas", 10), command=self.toggle_advanced_timing, checkbox_width=18, checkbox_height=18, corner_radius=4, border_width=1, fg_color="#00CC00", hover_color="#009900")
        ...
        self.ultra_button = ctk.CTkButton(buttons_frame, text="ULTRA MODE", height=40, font=("Consolas", 12, "bold"), corner_radius=6, fg_color="#CC0000", hover_color="#990000", command=self.toggle_ultra_mode)
        self.ultra_button.pack(fill="x", pady=4)

        self.autoburst_button = ctk.CTkButton(buttons_frame, text="AUTO-BURST [F5]", height=40, font=("Consolas", 12, "bold"), corner_radius=6, fg_color="#444444", hover_color="#555555", command=self.toggle_auto_burst)
        self.autoburst_button.pack(fill="x", pady=4)

        self.random_button = ctk.CTkButton(buttons_frame, text="RANDOM CONFIG", height=40, font=("Consolas", 12, "bold"), corner_radius=6, fg_color="#444444", hover_color="#555555", command=self.randomize_config)
    ...
    def create_footer(self):
        footer_label = ctk.CTkLabel(self, text="[F3] CAPTURE | [F2] BURST | [F5] AUTO", font=("Consolas", 9), text_color="#666666")
    ...
    def toggle_advanced_timing(self):
        ...
        try:
            ...
            desired_state = self.advanced_checkbox.get()
    ...
    def set_burst_state(self, bursting):
        if bursting:
            ...
            # Reset burst clicks counter when burst stops
            self.clicker.current_burst_clicks = 0
    ...
    def update_coords(self):
        ...
        self.coords_label.configure(text=f"TARGET: [ {x:04d} , {y:04d} ]", text_color="#00CC00")
    ...
    def update_stats(self):
        # Update the stats label with total clicks and current burst clicks
        total_clicks = self.clicker.total_clicks_sent
        burst_clicks = self.clicker.current_burst_clicks
        self.stats_label.configure(text=f"CLICKS: {total_clicks} / {burst_clicks}")
